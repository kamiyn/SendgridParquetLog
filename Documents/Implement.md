...
4. 別 `Task` で `ExecuteCompactionAsync` を開始。日ごと・バッチごとに `v3raw` を読み、時間単位で束ねて `v3compaction` へ書き出す。
   ...
   - バッチ内のファイル読み込みは最大 2 ファイルを先読みする producer/consumer 方式（bounded channel）。読み側が例外で倒れても書き側が満杯チャネルで永久ブロックしないよう、双方を連動キャンセルし本来の失敗原因を送出する（デッドロック防止）。
   - 出力後に Parquet として再読込できるか **Verify** し、成功して初めて元の生ファイルを削除（検証失敗時は出力側を削除して元を残す）。
5. 実行中は**ロックのハートビート**（`LockDuration/6 = 5 分`ごと）を並行して回す。**本体が前進している間だけロックを延長**し、前回延長（初回ロック取得を含む）から進捗（`RunStatus.LastUpdated`）が一度も進んでいなければ延長しない。前進が止まると延長も止まり、無延長のまま `LockDuration`（30 分）が経過＝ロックが期限切れになった時点で、二重実行防止のためコンパクションをキャンセルする。延長要求が連続 3 回失敗した場合（`Abandoned`）も同様にキャンセルする。これにより、ハングしたプロセスがハートビートでロックを恒久保持し次回 run が回復不能になるのを防ぐ。
...
stateDiagram-v2
    ...
    Held --> Held: ExtendLockForHeartbeatAsync ハートビート5分毎 [If-Match:etag] 進捗がある間のみ延長（所有者一致）
    Held --> Held: ExtendLockExpirationAsync バッチ境界 [If-Match:etag] 所有者一致で延長
    ...
    Held --> Expired: 進捗停止で延長せず LockDuration 経過（自然に期限切れ）
