...
Slack 通知を有効にする場合は以下を Repository Secrets に設定してください（Webhook URL はチャネル投稿権限を持つため Variables ではなく Secrets 推奨）:

| シークレット名 | 説明 |
|---------------|------|
| SLACKNOTIFIER__WARNINGWEBHOOKURL | 警告通知用 Slack Incoming Webhook URL（任意） |
| SLACKNOTIFIER__INFORMATIONWEBHOOKURL | 情報通知用 Slack Incoming Webhook URL（任意） |
